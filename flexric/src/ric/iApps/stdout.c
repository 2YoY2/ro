...
//    print_kpm_stats(&data->kpm_ind);
    ...
    // assert(0!=0 && "Not implemented");
  } else if (data->type == ISAC_STATS_V0){
    isac_ind_msg_t const* msg = &data->isac.msg;
    for(uint32_t i = 0; i < msg->len_snap; ++i){
      isac_csi_meta_t const* m = &msg->snap[i].meta;
      printf("ISAC CSI snap: seq %u rnti %04x sfn %u.%u rx_ant %u ue_ports %u sc %u snr %d dB dropped %u\n",
             m->seq, m->rnti, m->frame, m->slot, m->nb_rx_ant, m->nb_ue_ports, m->nb_sc,
             m->wb_snr_db, msg->dropped_snap);
    }
  } else {
