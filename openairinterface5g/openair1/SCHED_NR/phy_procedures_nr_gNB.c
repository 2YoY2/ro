...
static void handle_srs(fsn_t now, PHY_VARS_gNB *gNB, const NR_gNB_SRS_job_t *srs, nfapi_nr_srs_indication_pdu_t *srs_indication)
{
  ...
  if (srs_est >= 0) {
    // ISAC: expose this SRS channel estimate H[k, r] to the E2 agent (ISAC SM)
    const uint16_t m_srs = srs_pdu->srs_parameters_v4.srs_bandwidth_size;
    const uint32_t first_subcarrier =
        (frame_parms->first_carrier_offset - (ofdm_symbol_size >> 1)) + srs_pdu->bwp_start * NR_NB_SC_PER_RB;
    uint32_t start_sc_off[N_ap];
    for (int p = 0; p < N_ap; p++)
      start_sc_off[p] = first_subcarrier + srs_info.k_0_p[p][0];

    const nr_isac_csi_meta_t meta = {
        .tstamp_us = nr_isac_time_now_us(),
        .ul_carrier_freq_hz = frame_parms->ul_CarrierFreq,
        .nb_sc = m_srs * NR_NB_SC_PER_RB,
        .first_sc = srs_pdu->bwp_start * NR_NB_SC_PER_RB + srs_info.k_0_p[0][0],
        .scs_hz = frame_parms->subcarrier_spacing,
        .frame = now.f,
        .slot = now.s,
        .rnti = srs_pdu->rnti,
        .wb_snr_db = snr,
        .ta_offset = timing_advance_offset,
        .nb_prb_carrier = frame_parms->N_RB_UL,
        .nb_prb_srs = m_srs,
        .nb_symb_srs = N_symb_SRS,
        .nb_rx_ant = nb_antennas_rx,
        .nb_ue_ports = N_ap,
        .comb_size = 2 << srs_pdu->comb_size,
    };
    nr_isac_csi_publish(&meta,
                        (const int16_t *)&srs_estimated_channel_freq[0][0][0],
                        (size_t)N_ap * ofdm_symbol_size * N_symb_SRS,
                        (size_t)ofdm_symbol_size * N_symb_SRS,
                        start_sc_off);
  }
